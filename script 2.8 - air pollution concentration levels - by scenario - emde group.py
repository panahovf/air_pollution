...
df_frac_contribution = df_frac_contribution[df_frac_contribution['GU_A3'].isin(v_countrycodes)]
...
# print
print(df_overall['population'].sum()/10**9)   # 6.557257396551305
...
df_cp_power = df_cp_power[df_cp_power['Region'].isin(v_countrycodes)]
df_nz_power = df_nz_power[df_nz_power['Region'].isin(v_countrycodes)]
...
df_concentration_cp_annual.to_excel('2 - output/script 2.1.2 - air pollition concentration levels - by scenario - emde/1.1 - annual concentration levels - current policy.xlsx', index = False)
df_concentration_nz_annual.to_excel('2 - output/script 2.1.2 - air pollition concentration levels - by scenario - emde/1.2 - annual concentration levels - netzero 1.5C 50% adjsuted.xlsx', index = False)
